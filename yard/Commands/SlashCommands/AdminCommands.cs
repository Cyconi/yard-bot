...
namespace yard.Commands.SlashCommands
{
    internal class AdminCommands : ApplicationCommandModule
    {
        ...
        public async Task GenerateKey(InteractionContext ctx, [Option("user", "User to receive key")] DiscordUser user)
        {
            ...
            {
                CLog.L("Server", ctx.User, $"`<@{ctx.User.Id}>` tried calling /key command with insufficient permissions", Channel.command);
                await ctx.CreateResponseAsync("You do not have access to this command", true);
                return;
            }
            CLog.L("Server", ctx.User, $"`<@{ctx.User.Id}>` called /key command", Channel.command);
            ...
            CLog.L("Server", ctx.User, $"Generated Key: {key}", Channel.command);
        ...
        {
            CLog.L("Server", ctx.User, $"`<@{ctx.User.Id}>` called /listinventories command", Channel.command);

            if (!Program.adminIds.Contains(ctx.User.Id))
            {
                CLog.L("Server", ctx.User, $"`<@{ctx.User.Id}>` attempted /listinventory with insufficient permissions", Channel.command);
                await ctx.CreateResponseAsync("You do not have access to this command.", true);
                return;
            }
            ...
            CLog.L("Server", ctx.User, "Listed all user inventories.", Channel.command);
        }
        ...
        public async Task ListUserInventory(InteractionContext ctx, [Option("user", "User whose inventory you want to check")] DiscordUser user)
        {
            if (!Program.adminIds.Contains(ctx.User.Id))
            {
                CLog.L("Server", ctx.User, $"`<@{ctx.User.Id}>` called /userinventory for `<@{user.Id}>` with insufficient permissions", Channel.command);
                await ctx.CreateResponseAsync("You do not have access to this command.", true);
                return;
            }
            CLog.L("Server", ctx.User, $"`<@{ctx.User.Id}>` called /userinventory for `<@{user.Id}>`", Channel.command);
            ...
            CLog.L("Server", ctx.User, $"Displayed inventory for `<@{user.Id}>`", Channel.command);
        ...
        public async Task ClearUserInventory(InteractionContext ctx, [Option("user", "User whose inventory you want to clear")] DiscordUser user)
        {
            if (!Program.adminIds.Contains(ctx.User.Id))
            {
                CLog.L("Server", ctx.User, $"`<@{ctx.User.Id}>` called /clearinventory for `<@{user.Id}>` with insufficient permissions", Channel.command);
                await ctx.CreateResponseAsync("You do not have access to this command.", true);
                return;
            }
            CLog.L("Server", ctx.User, $"`<@{ctx.User.Id}>` called /clearinventory for <@{user.Id}>", Channel.command);

            var inventory = InventoryManager.Get(user.Id);

            if (inventory == null)
            {
                CLog.L("Server", ctx.User, $"Attempted to clear inventory for `<@{user.Id}>` but no inventory was found.", Channel.command);
            ...
            CLog.L("Server", ctx.User, $"Successfully cleared inventory for `<@{user.Id}>`", Channel.command);
        ...
        public async Task DeleteKey(InteractionContext ctx, [Option("key", "The key to delete")] string key)
        {
            if (!Program.adminIds.Contains(ctx.User.Id))
            {
                CLog.L("Server", ctx.User, $"`<@{ctx.User.Id}>` called /deletekey for {key} with insufficient permissions", Channel.command);
                await ctx.CreateResponseAsync("You do not have access to this command.", true);
                return;
            }
            CLog.L("Server", ctx.User, $"`<@{ctx.User.Id}>` called /deletekey for {key}", Channel.command);
            ...
            if (keyObject == null)
            {
                CLog.L("Server", ctx.User, $"Failed to delete key {key}: Key not found.", Channel.command);
            ...
            CLog.L("Server", ctx.User, $"Successfully deleted key {key} from `<@{inventory.UserId}>` inventory.", Channel.command);
        ...
        public async Task FindKey(InteractionContext ctx, [Option("key", "The key to find")] string key)
        {
            if (!Program.adminIds.Contains(ctx.User.Id))
            {
                CLog.L("Server", ctx.User, $"`<@{ctx.User.Id}>` called /findkey for {key} with insufficient permissions", Channel.command);
                await ctx.CreateResponseAsync("You do not have access to this command.", true);
                return;
            }
            CLog.L("Server", ctx.User, $"`<@{ctx.User.Id}>` called /findkey for {key}", Channel.command);
            ...
            if (keyObject == null)
            {
                CLog.L("Server", ctx.User, $"Key {key} not found in any inventory.", Channel.command);
            ...
            CLog.L("Server", ctx.User, $"Displayed full details of key {key}.", Channel.command);
        ...
        public async Task PurgeKeys(InteractionContext ctx)
        {
            if (!Program.adminIds.Contains(ctx.User.Id))
            {
                CLog.L("Server", ctx.User, $"`<@{ctx.User.Id}>` called /purgekeys command with insufficient permissions", Channel.command);
                await ctx.CreateResponseAsync("You do not have access to this command.", true);
                return;
            }

            CLog.L("Server", ctx.User, $"`<@{ctx.User.Id}>` called /purgekeys command", Channel.command);
            ...
            CLog.L("Server", ctx.User, $"Purged {removedCount} non-redeemed keys from inventories.", Channel.command);
        ...
        public async Task GenerateMassKeys(InteractionContext ctx)
        {
            if (!Program.adminIds.Contains(ctx.User.Id))
            {
                CLog.L("Server", ctx.User, $"`<@{ctx.User.Id}>` called /masskey command with insufficient permissions", Channel.command);
                await ctx.CreateResponseAsync("You do not have access to this command.", true);
                return;
            }
            CLog.L("Server", ctx.User, $"`<@{ctx.User.Id}>` called /masskey command", Channel.command);
            ...
            foreach (var inventory in InventoryManager.Inventories)
            {
                ...
                {
                    CLog.L("Server", ctx.User, $"Failed to generate key for `<@{inventory.UserId}>`", Channel.command);
                    continue;
                }

                keysGenerated++;
                CLog.L("Server", ctx.User, $"Generated Key: {newKey} for `<@{inventory.UserId}>`", Channel.command);
                ...
                try
                {
                    ...
                    CLog.L("Server", ctx.User, $"Sent DM to `<@{inventory.UserId}>` about new key {newKey}.", Channel.command);
                }
                catch (Exception ex)
                {
                    CLog.L("Server", ctx.User, $"Failed to send DM to `<@{inventory.UserId}>`. Error: {ex.Message}", Channel.error);
                }
            }
        ...
        public async Task MakeInventories(InteractionContext ctx)
        {
            if (!Program.adminIds.Contains(ctx.User.Id))
            {
                CLog.L("Server", ctx.User, $"`<@{ctx.User.Id}>` called /makeinventories command with insufficient permissions", Channel.command);
                await ctx.CreateResponseAsync("You do not have access to this command.", true);
                return;
            }
            CLog.L("Server", ctx.User, $"`<@{ctx.User.Id}>` called /makeinventories command", Channel.command);
            ...
            foreach (var member in members.Where(m => m.Roles.Contains(role)))
            {
                if (InventoryManager.Get(member.Id) == null)
                {
                    ...
                    {
                        CLog.L("Server", ctx.User, $"Failed to create inventory for `<@{member.Id}>`", Channel.command);
                        continue;
                    }

                    CLog.L("Server", ctx.User, $"Created inventory for `<@{member.Id}>`", Channel.command);
                    createdCount++;
                }
            }
